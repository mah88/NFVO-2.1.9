...
public class NSRUtils {
    ...
    public static NetworkServiceRecord createNetworkServiceRecord(NetworkServiceDescriptor networkServiceDescriptor) throws NotFoundException, BadFormatException {
        ...
        if(networkServiceDescriptor.getVnffgd().size()!=0) {
            for (VNFForwardingGraphDescriptor vnfForwardingGraphDescriptor : networkServiceDescriptor.getVnffgd()) {
                {
                    VNFForwardingGraphRecord vnffgr = NSRUtils.createVNFForwardingGraphRecord(vnfForwardingGraphDescriptor);
    ...
    // Creating VNFFG Record
    public static VNFForwardingGraphRecord createVNFForwardingGraphRecord(VNFForwardingGraphDescriptor vnffgDescriptor) {
      ...
      //  vnfforwardingGraphRecord.setParent_ns(new NetworkServiceRecord());
       ...
       /*  for(VirtualLinkRecord vlr: networkServiceRecord.getVlr()){
        ...
        vnfforwardingGraphRecord.setStatus(Status.NULL);
       ...
       for (NetworkForwardingPath networkForwardingPath : vnffgDescriptor.getNetwork_forwarding_path()) {
            NetworkForwardingPath newNetworkForwardingPath = new NetworkForwardingPath();
            Policy newPolicy = new Policy();
            ACL_Matching_Criteria newACL = new ACL_Matching_Criteria();
            newACL.setDestinationIP(networkForwardingPath.getPolicy().getMatchingCriteria().getDestinationIP());
            ...
            newACL.setDestinationPort(networkForwardingPath.getPolicy().getMatchingCriteria().getDestinationPort());
            ...
            newNetworkForwardingPath.setPolicy(newPolicy);
             ...
             Map<String,String> newConnection=new HashMap<String,String>();

            for (Map.Entry<String, String> entry : networkForwardingPath.getConnection().entrySet())
            {
                newConnection.put(entry.getKey(),entry.getValue());
            }
            newNetworkForwardingPath.setConnection(newConnection);
      ...
      /*  if(networkServiceRecord.getVnfr().size()!=0){
        ...
        */
        ...
        vnfforwardingGraphRecord.setNumber_of_virtual_links(vnfforwardingGraphRecord.getDependent_virtual_link().size());
